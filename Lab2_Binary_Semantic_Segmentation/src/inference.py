...
def infer(model_name, Model, batch_size=10):


    test_dataset = oxford_pet.SimpleOxfordPetDataset(root="./dataset/oxford-iiit-pet", mode='test')
    test_dataloader = DataLoader(dataset=test_dataset, batch_size=batch_size)

    unn_model = Model().cuda()
    unn_model.load_state_dict(torch.load(f"./saved_models/{model_name}"))
...
if __name__ == '__main__':
    ...
    # infer("resnet34_unet_weights_20_3e-05_d.pth", UNet)
    infer("resnet34_unet_weights_20_1e-05_zn.pth", ResNet34_UNet)